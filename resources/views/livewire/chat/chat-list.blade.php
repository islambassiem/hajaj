<div
    x-data="{ type: 'all', query: $wire.entangle('query') }"
    x-init="setTimeout(() => {
            ...
            @if ($conversations)
                ...
                @foreach ($conversations as $key => $conversation)
                    <li id="conversation-{{ $conversation->id }}" wire:key="{{ $conversation->id }}"
                        class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 {{ $conversation->id == $selectedConversation?->id ? 'bg-gray-200' : '' }}">
                        <a href="#" class="shrink-0">
                            <x-avatar src="https://picsum.photos/id/{{ $key }}/200/200" />
                        </a>
